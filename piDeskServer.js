...
wss.on('connection', function connection(ws) {
    ...
    ws.on('message', function incoming(message) {
        if (message == 'powerToggle') {
            ...
            delayedWrite(pinConfig.mute, powerState, function () { }, 0);
            delayedWrite(pinConfig.power, powerState, function () { }, 250);
            delayedWrite(pinConfig.mute, !powerState, function () { }, 250);
        }
...
function broadcastPowerState() {
    wss.broadcast(`powerState:${powerState}`);
}

function broadcastMuteState() {
    wss.broadcast(`muteState:${muteState}`);
}
